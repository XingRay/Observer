 ...
 *
 */
class ObserverableMap<K, V : Observable> {

    private val map by lazy { mutableMapOf<K, V?>() }
    private val observers by lazy { SetMap2<K, Executor?, Observer<V>>() }

    fun addObserver(key: K, executor: Executor, observer: Observer<V>) {
        observers.add(key, executor, observer)
    }

    fun removeObserver(observer: Observer<V>) {
